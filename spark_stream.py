...
def create_keyspace(session):
    session.execute("""
         CREATE KEYSPACE IF NOT EXISTS spark_streams WITH replication = {'class':
...
def create_table(session):
    session.execute("""
         CREATE TABLE IF NOT EXISTS spark.streams.created_users (
                    id UUID PRIMARY KEY,
                    first_name TEXT,
                    last_name TEXT,
                    gender TEXT,
...
def insert_data(session, **kwargs):
    ...
    try:
        session.execute("""
              INSERT INTO spark_streams.created_users(user_id, first_name, last_name, gender, address, postcode, email, 
...
def create_spark_connection():     #https://mvnrepository.com/artifact/com.datastax.spark/spark-cassandra-connector_2.13/3.4.1
    s_conn = None
